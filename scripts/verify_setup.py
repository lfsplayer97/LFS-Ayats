...
def check_virtual_environment() -> bool:
    ...
    if in_venv:
        print_check(True, "Virtual environment detected", 
...
def check_core_dependencies() -> bool:
    ...
    if not missing:
        print_check(True, "Core dependencies installed",
                   f"Verified {len(required_packages)} packages")
        return True
    else:
        print_check(False, "Missing dependencies",
...
def check_package_installed() -> bool:
    ...
    if result.returncode != 0:
        print_check(False, "Package 'lfs-ayats' not installed",
                   "Run: pip install -e .")
        return False
    ...
    try:
        ...
        if is_editable:
            print_check(True, "Package installed in editable mode",
                       f"Location: {location}")
            return True
        else:
            print_check(False, "Package installed but not in editable mode",
                       "Run: pip install -e . to enable auto-reload")
            return False
            
    except ImportError as e:
        print_check(False, "Cannot import 'src' module",
                   f"ERROR: {e}. Try: pip install -e .")
        return False
...
def check_config_file() -> bool:
    ...
    if config_path.exists():
        print_check(True, "Configuration file exists",
                   f"Location: {config_path.absolute()}")
        return True
    elif example_path.exists():
        print_check(False, "Configuration file not found",
                   f"Copy {example_path} to {config_path}")
        return False
    else:
        print_check(False, "Configuration files missing",
                   "Cannot find config.example.yaml")
        return False
...
def check_tests_can_run() -> bool:
    ...
    if result.returncode == 0:
        ...
        print_check(True, "Tests can be executed",
                   f"Found tests: {last_line}")
        return True
    else:
        print_check(False, "Cannot execute tests",
                   "ERROR: pytest failed")
        return False
...
def check_project_structure() -> bool:
    ...
    if not missing_dirs and not missing_files:
        print_check(True, "Project structure is complete",
        ...
        print_check(False, "Project structure incomplete",
                   "; ".join(errors))
        return False
...
def print_summary(checks: dict) -> None:
    ...
    if passed == total:
        print("\033[92m✓ All checks passed! Your environment is properly configured.\033[0m")
        print()
        print("You can now:")
